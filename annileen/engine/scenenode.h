...
namespace annileen
{
	...
	public:
		...
		T* getModule() const
		{
			...
			{
				return dynamic_cast<T*>(moduleIt->second);
			}
		...
		template <class T>
		bool addModule(T* module)
		{
			SceneNodeModulePtr sceneNodeModule = dynamic_cast<SceneNodeModulePtr>(module);
		...
		bool removeModule()
		{
			...
			T* module = dynamic_cast<T*>(moduleIt->second);
